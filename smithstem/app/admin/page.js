...
export default function AdminDashboard() {
  ...
  return (<div><Header role="admin" onSignOut={signOut} /><main className="mx-auto max-w-5xl px-4 py-4"><nav className="mb-6 flex gap-2">{[["approvals", `Bonus approvals${pending.length ? ` (${pending.length})` : ""}`], ["creators", "Manage creators"], ["payments", "Payments register"]].map(([key, label]) => (<button key={key} onClick={() => setTab(key)} className={`rounded-xl px-4 py-2 text-sm font-semibold ${tab === key ? "bg-accent text-white" : "bg-white text-slate-500 border border-slate-200"}`}>{label}</button>))}</nav>{msg && <p className="mb-4 text-sm text-accent">{msg}</p>}{tab === "approvals" && (<section className="card"><h2 className="mb-3 font-semibold">Pending bonus claims ({pending.length})</h2><div className="space-y-2">{pending.length === 0 && <p className="text-sm text-slate-400">Nothing waiting on you.</p>}{pending.map((c) => (<div key={c.id} className="rounded-xl border border-slate-100 px-4 py-3"><div className="flex items-center justify-between"><div><p className="font-medium">{c.creators?.profiles?.full_name}</p><p className="text-sm text-slate-500">{c.claim_date} · {Number(c.views).toLocaleString()} views · +{fmtNaira(bonusForViews(c.views, tiers))}</p></div><div className="flex gap-2"><button className="btn-secondary text-xs" onClick={() => reviewClaim(c, "rejected")}>Reject</button><button className="btn-primary text-xs" onClick={() => reviewClaim(c, "approved")}>Approve</button></div></div>{c.video_url && (<a href={c.video_url} target="_blank" rel="noopener noreferrer" className="mt-1 block text-xs text-accent underline">Open submitted video</a>)}</div>))}</div></section>)}{tab === "creators" && (<section><div className="mb-4 flex items-center gap-3"><label className="text-sm font-medium text-slate-500">Editing month:</label><input className="input w-auto" type="month" value={ym} onChange={(e) => setYm(e.target.value)} /></div><div className="card divide-y divide-slate-100">{creators.length === 0 && <p className="text-sm text-slate-400">No creators yet.</p>}{creators.map((c) => { const claims = claimsByCreator[c.id] || []; const vidCount = videoCountByCreator[c.id] || 0; return (<div key={c.id} className="py-3 first:pt-0 last:pb-0"><div className="flex items-center justify-between"><button className="text-left" onClick={() => openCreator(c)}><p className="font-semibold text-accent underline decoration-dotted">{c.profiles?.full_name}</p><p className="text-xs text-slate-400">Base pay ₦{Number(c.base_pay).toLocaleString()}/mo · {vidCount} video{vidCount !== 1 ? "s" : ""} in {ym} · {c.status}</p></button><button className="btn-secondary text-xs" onClick={() => logForCreator(c)}>+ Log video</button></div>{claims.length > 0 && (<div className="mt-2 space-y-1">{claims.map((claim) => (<div key={claim.id} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-1.5 text-xs"><span>{claim.claim_date} · {Number(claim.views).toLocaleString()} views · <span className={`badge ${STATUS_STYLE[claim.status]}`}>{claim.status}</span></span><button className="text-red-500 underline" onClick={() => deleteClaim(claim.id, c.id, ym)}>Delete</button></div>))}</div>)}</div>); })}</div></section>)}{tab === "payments" && (<section><div className="mb-4 flex items-center gap-3"><label className="text-sm font-medium text-slate-500">Month:</label><input className="input w-auto" type="month" value={ym} onChange={(e) => setYm(e.target.value)} /></div><div className="card overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-slate-100 text-left text-xs uppercase text-slate-400"><th className="py-2 pr-3">Creator</th><th className="pr-3">Base pay</th><th className="pr-3">Videos</th><th className="pr-3">Rate/post</th><th className="pr-3">Earned base</th><th className="pr-3">Perf. bonus</th><th className="pr-3">Referral</th><th className="pr-3">OOP</th><th className="pr-3">Total</th><th className="pr-3">Status</th></tr></thead><tbody>{payments.map((p) => { const basePay = p.creators?.base_pay || 0; const perPost = Math.round(rate(basePay)); const vids = videoCountByCreator[p.creator_id] || Math.round(p.base_amount / (perPost || 1)); return (<tr key={p.id} className="border-b border-slate-50"><td className="py-2 pr-3"><button className="text-accent underline decoration-dotted" onClick={() => openCreator(creators.find((c) => c.id === p.creator_id))}>{p.creators?.profiles?.full_name}</button></td><td className="pr-3">{fmtNaira(basePay)}</td><td className="pr-3">{vids} / 62</td><td className="pr-3">{fmtNaira(perPost)}</td><td className="pr-3">{fmtNaira(p.base_amount)}</td><td className="pr-3">{fmtNaira(p.perf_bonus)}</td><td className="pr-3"><input className="input w-20" defaultValue={p.referral_bonus} onBlur={(e) => savePaymentField(p, "referral_bonus", e.target.value)} /></td><td className="pr-3"><input className="input w-20" defaultValue={p.oop_expense} onBlur={(e) => savePaymentField(p, "oop_expense", e.target.value)} /></td><td className="pr-3 font-semibold">{fmtNaira(p.total_payable)}</td><td className="pr-3"><select className="input w-28" defaultValue={p.payment_status} onChange={(e) => savePaymentField(p, "payment_status", e.target.value)}><option value="Pending">Pending</option><option value="Paid">Paid</option><option value="Held">Held</option></select></td></tr>); })}{payments.length === 0 && (<tr><td colSpan={10} className="py-4 text-center text-slate-400">No payment rows for {ym} yet — created as videos and bonuses are logged and approved.</td></tr>)}</tbody></table></div></section>)}{selectedCreator && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" onClick={() => setSelectedCreator(null)}><div className="max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6" onClick={(e) => e.stopPropagation()}><div className="mb-4 flex items-start justify-between"><div><h3 className="font-display text-xl font-bold">{selectedCreator.profiles?.full_name}</h3><p className="text-sm text-slate-500">{selectedCreator.profiles?.email} · {selectedCreator.profiles?.phone || "—"}</p></div><button className="text-slate-400 hover:text-ink" onClick={() => setSelectedCreator(null)}>✕</button></div><div className="mb-4 grid grid-cols-2 gap-3 text-sm"><div className="rounded-xl bg-slate-50 p-3"><p className="text-xs font-semibold uppercase text-slate-400">Base pay</p><p className="font-semibold">{fmtNaira(selectedCreator.base_pay)}/mo</p></div><div className="rounded-xl bg-slate-50 p-3"><p className="text-xs font-semibold uppercase text-slate-400">Status</p><p className="font-semibold capitalize">{selectedCreator.status}</p></div></div><div className="mb-4 rounded-xl bg-slate-50 p-3 text-sm"><p className="mb-1 text-xs font-semibold uppercase text-slate-400">Bank</p><p>{selectedCreator.bank_name || "—"} · {selectedCreator.acct_num || "—"} · {selectedCreator.acct_name || "—"}</p></div>{selectedCreator.contract_file_url && (<a href={selectedCreator.contract_file_url} target="_blank" rel="noopener noreferrer" className="mb-4 inline-block text-sm text-accent underline">Open signed contract</a>)}<div className="mb-4"><p className="mb-2 text-xs font-semibold uppercase text-slate-400">All videos ({selectedCreatorVideos.length})</p><div className="max-h-48 space-y-1 overflow-y-auto">{selectedCreatorVideos.length === 0 && <p className="text-xs text-slate-400">None logged.</p>}{selectedCreatorVideos.map((v) => (<div key={v.id} className="flex items-center justify-between rounded-lg border border-slate-100 px-3 py-1.5 text-xs"><span>{v.log_date} · Post {v.post_number} · <span className="text-slate-400">by {v.logged_by}</span></span><div className="flex gap-2">{v.tiktok_url && <a href={v.tiktok_url} target="_blank" rel="noopener noreferrer" className="text-accent underline">TT</a>}{v.insta_url && <a href={v.insta_url} target="_blank" rel="noopener noreferrer" className="text-accent underline">IG</a>}</div></div>))}</div></div><div><p className="mb-2 text-xs font-semibold uppercase text-slate-400">All bonus claims ({selectedCreatorClaims.length})</p><div className="max-h-48 space-y-1 overflow-y-auto">{selectedCreatorClaims.length === 0 && <p className="text-xs text-slate-400">None submitted.</p>}{selectedCreatorClaims.map((c) => (<div key={c.id} className="rounded-lg border border-slate-100 px-3 py-1.5 text-xs"><div className="flex items-center justify-between"><span>{c.claim_date} · {Number(c.views).toLocaleString()} views · <span className={`badge ${STATUS_STYLE[c.status]}`}>{c.status}</span></span></div>{c.video_url && <a href={c.video_url} target="_blank" rel="noopener noreferrer" className="text-accent underline">Open video</a>}</div>))}</div></div></div></div>)}</main></div>);
}
