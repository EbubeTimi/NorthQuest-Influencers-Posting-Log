...
export default function OnboardingPage() {
  ...
  const [form, setForm] = useState({
    fullName: "", phone: "", tiktok: "", insta: "", bankName: "", acctNum: "", acctName: "",
  ...
  async function submit() {
    ...
    if (existingCreator) {
      ...
      const { data: creator, error: creatorErr } = await supabase.from("creators").insert({ profile_id: user.id, business_id: business.id, bank_name: form.bankName, acct_num: form.acctNum, acct_name: form.acctName, contract_signed_at: new Date().toISOString(), tiktok_profile_url: form.tiktok.trim() || null, insta_profile_url: form.insta.trim() || null }).select().single();
    ...
    // Copies the contract and account details into the real Google Drive
    // Smith and Ella already use. Never blocks getting into the dashboard —
    // Postgres is already saved either way, and this is best-effort on top.
    try {
      await supabase.functions.invoke("drive-sync", {
        body: {
          creatorId, contractStoragePath: path,
          appEmail: form.appEmail.trim() || null, appEmailPassword: form.appEmailPassword || null,
          instagramHandle: form.instagramHandle.trim() || null, instagramPassword: form.instagramPassword || null,
          tiktokHandle: form.tiktokHandle.trim() || null, tiktokPassword: form.tiktokPassword || null,
        },
      });
    } catch (_) { /* best-effort — see comment above */ }
    setForm((f) => ({ ...f, appEmailPassword: "", instagramPassword: "", tiktokPassword: "" }));
