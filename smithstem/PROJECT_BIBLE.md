...
Latest user storage selection: Google Drive for temporary introductions. Destination setup is complete (new private owner-only folder, metadata verified); website upload/attachment/playback integration remains blocked by separate runtime authorization and unverified test-environment boundaries. No video upload/deletion, source Form/Sheet mutation or deployment. Folder metadata is in a local private handoff outside the repository. See `../audit/tdt-unified-creator-ops/RECRUITMENT_STORAGE_ASSESSMENT.md`. Final UI/UX passes follow completion of the flows; this supersedes the provider-pending recommendation below, not the proof/production approval gates.

Recruitment revision 7 supersedes the review status below: user accepted this flow for now, deferred final UI/colour polish, and explicitly retained History. Business headings are below the back control. The next-phase review shortcut uses the existing recipient trial/evidence/onboarding transitions; 23 fresh browser checks plus 2 deep-link checks cover correction/retry, manager checks and exactly one local membership. The same page's 40 recruitment regression checks also pass. See `../evidence/flows/Recruitment_Prototype/REVISION_7.md`. This is not production approval or a real membership/storage write. Private Supabase is the proposed storage default, Drive is an alternative; user choice, retention policy and live permissions remain pending. Existing legacy Storage/Drive/Sheets code must not be reused blindly (see `../audit/tdt-unified-creator-ops/RECRUITMENT_STORAGE_ASSESSMENT.md`).

Recruitment revision 6 is the newest authority: five visible stage filters (Contacted, Cannot be reached, Unresponsive, Declined, Trial started); original evidence/onboarding state is preserved internally, not exposed as extra recruitment stages. Declined is applicant refusal, distinct from team rejection and excluded from Accepted summaries. The reason, recorder and contact history remain visible. New trials no longer get a scheduled end date; invitation expiry is separate. Brand renaming changes a display label keyed by stable business identity, never memberships, policy keys, links or historical snapshots. Application chrome is GrowthCooks Marketing Agency / Creator application, required marks are by labels, and existing automatic in-tab drafts are clearly distinguished from future durable autosave. Evidence: `../evidence/flows/Recruitment_Prototype/REVISION_6.md`. No new production grant, storage or firing workflow is implemented.
