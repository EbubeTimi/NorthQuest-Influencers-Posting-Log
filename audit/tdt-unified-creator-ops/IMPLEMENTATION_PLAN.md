...
The latest trial-flow observations are applied in prototype revision 5: dashboard weekly navigation, a separate anytime 10,000-view submission with screenshot, compact per-video/platform view inputs, and short pending/approved screens. Do not ask the user to repeat observations or restart the audit. This revision is trial-only and stops at onboarding-ready, not actual onboarding or an active-creator/admin dashboard.
...
5. **Build trial review workflow** — anytime one-video 10,000 submission plus weekly detection, screenshot/link proof, idempotent management notification, review queue, approve/keep-trial decision, onboarding unlock, and immutable audit event. Early milestones must not satisfy weekly reporting; record their actual submitted timestamp.
