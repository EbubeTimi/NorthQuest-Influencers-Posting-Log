...
Trial revision 7, recruitment-for-now, brand-onboarding behavior, and active-creator revision 16 behavior/flow are approved as the basis for the next phase. The next prototype is management contract administration, followed by the unified management dashboard. Read `DATA_STORAGE_ARCHITECTURE.md`, `RECRUITMENT_FLOW_CONTRACT.md`, and the latest conflict-register section before work. This is still prototype scope: no production UI, source Form/Sheet edits, production data change or deployment is authorized.
...
3. **Create managed business configuration** — period type/anchor/timezone, noon grace, Sheet destination, Apify schedule, per-business server-only credential reference, notifications, statuses, and safe versioned audit. Keep 10,000 as a protected policy.
...
8. **Build contract administration and onboarding persistence** — immutable brand-specific versions, authorized publish, in-app signature, generated signed PDF, exact corrections and audit history.
9. **Build the unified management dashboard** — videos, views, trial evidence, bonuses, payment preparation, creator lifecycle and audit connected to the same scoped records.
10. **Build CashDrive Inventory and Enquiries** — separate structured workspaces using the supplied form fields, tenant RLS, filters, ownership, follow-up, and history.
11. **Repair automations** — per-business weekly Sheets collation and Apify windows 1–14, 1–21, 1–month-end with separate NorthQuest/CashDrive/Aura Apify accounts, business-scoped secret resolution, ledgers, leases, retries, reconciliation, and cost guards.
12. **Implement the approved phone-first UI** — absorb approved prototype patterns into real components; remove prototype controls and mocks.
13. **Harden and prove** — RLS caller matrix, invitation abuse, session/revocation, uploads, rate limits, CSRF/CSP/cookies, secret and dependency scans, accessibility, load tests beyond 1,000 creators, preview deployment and rollback/incident runbooks.
...
- Trial revision 7 and active-creator revision 16 approval are recorded; final visual polish, production implementation authority and launch approval remain outstanding.
...
- Application upload retention, signed-document retention and draft retention need explicit product decisions before production persistence.
