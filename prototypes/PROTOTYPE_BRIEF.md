...
Recruitment revision 7 is current. The user accepted the recruitment flow for now and explicitly kept History; final colours/UI polish are deferred. Business headings now sit below Back. Continue the existing same-person journey at `recruitment.html#trial-onboarding` (review-only shortcut resets local examples): invitation → trial setup → logged link + screenshot → manager verification/correction → brand onboarding → membership-ready boundary. See `../evidence/flows/Recruitment_Prototype/REVISION_7.md`. The actual active dashboard remains the next separate review. Real storage provider/retention is not selected; legacy upload code exists but is not integrated or live-verified. Production implementation/deployment are still not approved. All earlier paragraphs below are revision history where they conflict.
